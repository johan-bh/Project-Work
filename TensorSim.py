...
with open('data/Simulated_Data_Tensor_allranks.pkl', 'rb') as f:
...
class Candemann_Parafac_module(nn.Module):
    def __init__(self, rank):
        ...
        self.rank = rank
        self.beta_0 = torch.nn.Parameter(torch.tensor(torch.rand(1),dtype=torch.float64))
        self.CP = {}
...
for test in test_names:#Looping over all tests seperately
    ...
    for epoch in range(n_iters):  # looping over all values in dataset for given rank and test
        ...
        try:
            y_true = response_var_df[test][keys[epoch]+ "rank"+ str(rank)]
        except:
            continue
